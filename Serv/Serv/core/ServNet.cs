...
public class ServNet
{
    ...
    private void ReceiveCb(IAsyncResult ar)
    {
        ...
        lock (conn)
        {
            ...
            {
                Console.WriteLine("收到 [" + conn.GetAddress() + "] 断开链接 " + e.Message);
                conn.Close();
            }
        }
    }
    ...
    private void HandleMsg(Conn conn, ProtocolBase protoBase)
    {
        ...
        //连接协议分发
        if (conn.player == null || name == "Logout")
        {
        ...
        {
            //MethodInfo mm = handlePlayerMsg.GetType().GetMethod(methodName);//判断handleplayermsg有没有传进来msg+name的方法，有就赋值个mm
            //if (mm == null)
            //{
